...
export function DateInput({ value, onChange }: DateInputProps) {
  ...
  const validateDate = (field: keyof DateParts, value: number): boolean => {
    if (
      (field === "day" && (value < new Date().getDate() || value > 31)) ||
      (field === "month" &&
        (value < new Date().getMonth() + 1 || value > 12)) ||
      (field === "year" && (value < new Date().getFullYear() || value > 9999))
    ) {
      return false;
    }
  ...
  return (
    <div className="flex items-center rounded-lg border px-1 text-md overflow-hidden min-h-[40px]">
      <input
        type="text"
        ref={dayRef}
        max={31}
        ...
        className="w-7 border-none p-0 text-center outline-none flex-1 bg-transparent"
        placeholder="D"
      />
      ...
      <input
        ...
        className="w-6 border-none p-0 text-center outline-none flex-1 bg-transparent"
        placeholder="M"
      />
      <span className="-mx-px opacity-20">/</span>
      <input
        ...
        className="w-12 border-none p-0 text-center outline-none flex-1 bg-transparent"
        placeholder="YYYY"
      />
    </div>
  );
